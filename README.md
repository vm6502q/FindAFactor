...
- `wheel_factorization_level` (default value: `11`): "Wheel" vs. "gear" factorization balances two types of factorization wheel ("wheel" vs. "gear" design) that often work best when the "wheel" is one prime smaller than the largest "gear." (In other words, like how `11` and `13` are consecutive primes, "wheels" and "gears" often work best with consecutive prime limits.) Optimized implementation for wheels is only available up to `13`.
